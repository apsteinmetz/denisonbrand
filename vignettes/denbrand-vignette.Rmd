...
den_text_sans {font-family: "Open Sans"}
</style>
```
```{r, include = FALSE}
...
## Adding Denison Logos to Plots

You can add Denison brand images (logos) to your plots using the `annotate_den_image()` function. This function allows you to place a PNG image from the package at any location on your plot.

Available images include:

- `denison_logo.png` - The full Denison University logo
- `denison_brick.png` - The Denison brick mark

```{r}
# Add the Denison logo to a plot
rank_data |>
  filter(college %in% c("Denison University", "Colgate University", "Lafayette College")) |>
  filter(year > 2010) |>
  ggplot(aes(x = year, y = rank, color = college)) +
  geom_line(linewidth = 1) +
  scale_y_reverse() +
  labs(title = "U.S. News Rankings with Logo",
       x = "Year",
       y = "Rank") +
  theme_den() +
  scale_color_den() +
  annotate_den_image("denison_logo.png", x = 2022, y = 50, scale = 0.6)
```

The `scale` parameter controls the size of the image. You can also use `hjust` and `vjust` parameters to adjust the alignment relative to the x and y coordinates.
...
body {font-family: "Open Sans"}
den_display_serif1 {font-family: "Lora"}
den_display_serif2 {font-family: "Crimson Pro"}
